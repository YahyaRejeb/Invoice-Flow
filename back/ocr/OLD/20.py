...
def compute_pu(montant_val, consumption_val):
    """Calculate P.U. (Prix Unitaire in millimes) using the formula:
    P.U.(X) = round(montant_X / consumption_X)

    Converts montant from TND (e.g. 40.027 TND) to millimes if needed,
    and returns an integer millimes value (e.g. 290). Guards against division by zero.
    """
    try:
        cons = int(consumption_val) if consumption_val is not None else 0
        if cons <= 0:
            return 0

        m = float(str(montant_val).replace(" ", "").replace(",", ".")) if montant_val is not None else 0.0
        if m <= 0:
            return 0

        # If montant is in Dinars (e.g., 40.027 TND), convert to millimes (40027 millimes)
        if m < 1000:
            m = m * 1000.0

        return int(round(m / cons))
    except (ValueError, TypeError, ZeroDivisionError):
        return 0


def extract_steg_calculation_table(gray_full, color_full=None):
    """Extract detailed tariff rows and summary rows from the supplied STEG layout."""
    result = {}
    confidence = {}
    g = STEG_TARIFF_TABLE

    # One OCR pass for the tariff table.
    table_words = _ocr_region_data(gray_full, (0.49, 0.59, 0.05, 0.42), psm=6)

    for period, y_range in g["tariff_rows"].items():
        # 1. Consommation OCR Extraction
        raw_cons, words_cons = _select_cell_text(table_words, gray_full.shape, y_range, g["consommation_x"])
        val_cons = _parse_cell_text(raw_cons, money=False)
        ag_cons = 1.0 if val_cons is not None and words_cons else 0.0

        if period == "nuit":
            val_cons, ag_cons = _targeted_single_cell(
                gray_full, y_range, g["consommation_x"], money=False, color_full=color_full
            )
        elif period == "soiree" and val_cons is None:
            val_cons, ag_cons = None, 0.0

        if val_cons is not None and int(float(val_cons)) < 10:
            val_cons = None
            ag_cons = 0.0

        if val_cons is None and period != "soiree":
            val_cons, ag_cons = _targeted_single_cell(
                gray_full, y_range, g["consommation_x"], money=False, color_full=color_full
            )

        cons_int = int(float(val_cons)) if val_cons is not None else 0
        result[f"consumption_{period}"] = cons_int
        confidence[f"consumption_{period}"] = ag_cons

        # 2. Montant OCR Extraction
        raw_mont, words_mont = _select_cell_text(table_words, gray_full.shape, y_range, g["montant_x"])
        val_mont = _parse_cell_text(raw_mont, money=True)
        ag_mont = 1.0 if val_mont is not None and words_mont else 0.0

        if period == "nuit":
            val_mont, ag_mont = _targeted_single_cell(
                gray_full, y_range, g["montant_x"], money=True, color_full=color_full
            )
        elif period == "soiree" and val_mont is None:
            val_mont, ag_mont = None, 0.0

        if val_mont is None and period != "soiree":
            val_mont, ag_mont = _targeted_single_cell(
                gray_full, y_range, g["montant_x"], money=True, color_full=color_full
            )

        mont_str = val_mont if val_mont is not None else "0.000"
        result[f"montant_{period}"] = mont_str
        confidence[f"montant_{period}"] = ag_mont

        # 3. P.U.(X) Calculated via Formula: P.U.(X) = round(montant_X / consumption_X)
        pu_calc = compute_pu(mont_str, cons_int)
        result[f"pu_{period}"] = pu_calc
        confidence[f"pu_{period}"] = 1.0 if cons_int > 0 and mont_str != "0.000" else 0.0

    # One OCR pass over the summary area. The numeric values are in the same left
    # column and are selected by their explicit row bands.
